...
_Last verified: 2026-07-06 (Workspaces v1) — 789 tests green (clippy + fmt clean), working tree clean, `main` @ 57f4b0a._
_**Workspaces v1 SHIPPED (live-verified end-to-end):** the tab is real — workspace list (name · path · scope-item counts · live git chip) with `+ add` (canonicalized, idempotent), rename, armed unregister (never touches the committed config — tested), focus-scope, and **runtime scope-chip rebuild** (the old startup-only caveat is closed). Detail by shape: **Repo** → Overview (branch · `↑n ↓n` · staged/unstaged/untracked · last commit) / Branches (click-to-checkout, DirtyWorkingTree refusal inline) / Status / Log; **Umbrella** (non-repo root with child repos) → the **sortable fleet table** (repo · branch · dirty · ahead/behind · age · path — verified over ~40 real repos in `~/vcs` with live data); **Plain** → scope items + "not a git repo". Mutation ceiling: checkout. `sid_core::git` + `sid-git` (git2, 33 real-repo tests, Mutex-wrapped handle — no unsafe). Commit/Diff/Actions/stash: v2, POC is the reference. sid-cap gained `--rclick`._
_**Terminal fidelity CLOSED (A/B-verified vs kitty over live SSH):** F4 — block elements U+2580-259F render as cell-snapped quads (`block_coverage`/`quad_edge` in session.rs; bit-identical shared edges make seams impossible), shades as alpha wash; box drawing stays glyphs (Caskaydia joins cleanly); truecolor pixel-identical; base-16 themed. Grid-enforced advances (E3) deliberately NOT built — no evidence needed. **Quick-connect: Enter now connects.** **sid-cap input engine:** headless seat advertises zero capabilities → clicks via persistent `scripts/cap-input/vptr.py` (venv bootstrap), typing under a keyboard-capability holder; `--env/--sleep/--xdg` for scripted live-SSH flows (recipe in the terminal-fidelity memory + docker sshd fixture)._

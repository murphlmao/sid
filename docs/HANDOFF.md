...
_Last verified: 2026-07-03 — 529 tests green (clippy + fmt clean), working tree clean, `main` @ 9ddcd07._
_**SSH/SFTP:** MobaXterm **multi-tab shell** — 🏠 home + per-connection session tabs, sidebar swaps between folder-grouped connections tree (inline rename, quick-connect filter) and the session's file browser (persisted dock side); multiple concurrent connections; reader/writer split fixed the terminal freeze; SFTP put can create. **Database:** query loop + schema tree + relationships diagram + generic `⭳ Export ▾`; **connection selector on the right**, folder grouping, inline rename. **Network v2:** Ports · Services (systemd) · Interfaces sub-tabs + filter + hidden-iface grouping. **Secrets:** keyring OR encrypted-file vault (argon2id+XChaCha20-Poly1305) OR memory — toggleable, unlock/create modal. **Keyboard:** Ctrl-modifier keymap registry + `Ctrl+K` command palette + `Ctrl+1..5` tabs + `Ctrl+Tab`/`Ctrl+T`/`Ctrl+W` session control; terminal-focus passes `Ctrl+<letter>` to the PTY (app variants use `Ctrl+Shift+<letter>` there). **Testing:** 529 unit/prop + docker pg/sshd integration (`scripts/test-*.sh`) + headless Xvfb smoke. Audits: [architecture](design/2026-07-02-architecture-audit.md) + [perf](design/2026-07-02-perf-audit.md). Agents drive the UI via `scripts/sid-shot.sh` + `scripts/sid-click.sh`._

...
_Last verified: 2026-07-02 — 427 tests green (clippy + fmt clean), working tree clean, `main` @ 3223bae. **Three tabs do real work.** SSH/SFTP: MobaXterm split session, cleaned browser toolbar, hidden-files toggle, kitty-parity font. Database: query loop + schema tree + relationships diagram (pop-out, click-through: table→run, column→WHERE scaffold) + generic `⭳ Export ▾` (far right). Network v2: **Ports · Services · Interfaces** sub-tabs, instant filter, systemd control (`sid_core::svc` + `sid-svcctl`), hidden-iface grouping. Store: folders + rename ops + `file_browser_side` (v3 migrations). Architecture audit passed ([design/2026-07-02-architecture-audit.md](design/2026-07-02-architecture-audit.md)). Visual debugging: `scripts/sid-shot.sh` + `scripts/sid-click.sh` (agents click-verify their own UI). **Awaiting Murphy:** ssh-v3 + keyboard mockup verdicts (`docs/mockups/2026-07-02-{ssh-v3-mobaxterm,keyboard-driven}.html`) → then the SSH multi-tab shell rebuild._

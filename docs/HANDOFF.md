...
_Last verified: 2026-07-02 — 486 tests green (clippy + fmt clean), working tree clean, `main` @ ccd9eef. **Three tabs do real work.** SSH/SFTP: MobaXterm split session, cleaned toolbar, hidden-files toggle, kitty font; **reader/writer shell split fixes a mutex-across-await freeze**; SFTP put can create files. Database: query loop + schema tree + relationships diagram (click-through: table→run, column→WHERE) + generic `⭳ Export ▾`. Network v2: Ports · Services (systemd, `sid-svcctl`) · Interfaces sub-tabs, filter, hidden-iface grouping. Store: folders + rename ops + `file_browser_side` (v3). **Secrets:** keyring OR encrypted-file vault (argon2id+XChaCha20-Poly1305) OR memory — toggleable, unlock/create modal, install-recommendation. **Testing:** 486 unit/prop + docker pg/sshd integration (`scripts/test-*.sh`) + headless Xvfb smoke. Audits: [architecture](design/2026-07-02-architecture-audit.md) (adapter pattern sound) + [perf](design/2026-07-02-perf-audit.md) (freeze fixed + 6 cleanups; terminal-grid memo deferred). Agents drive the UI via `scripts/sid-shot.sh` + `scripts/sid-click.sh`._

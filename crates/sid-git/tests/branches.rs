...
fn current_branch_matches_list_branches_current() {
    ...
    let provider = factory.open(dir.path()).unwrap();
    let listed_current =
        provider.list_branches().unwrap().into_iter().find(|b| b.is_current).unwrap();
...
fn list_branches_finds_a_second_branch() {
    ...
    let provider = factory.open(dir.path()).unwrap();
    let names: Vec<_> =
        provider.list_branches().unwrap().into_iter().map(|b| b.name).collect();
...
fn branch_head_oid_is_40_char_hex() {
    ...
    let branches = provider.list_branches().unwrap();
    for b in &branches {
        assert_eq!(b.head_oid.len(), 40, "OID should be 40 hex chars, got: {}", b.head_oid);
        assert!(
...
fn list_branches_on_unborn_repo_returns_empty() {
    ...
    assert!(branches.is_empty(), "unborn repo should have no local branches");
}
...
fn list_branches_handles_deeply_nested_name() {
    ...
    let repo = init_repo_with_initial_commit(dir.path());
    let head_commit = repo.head().unwrap().peel_to_commit().unwrap();
    repo.branch("fix/auth/token/refresh", &head_commit, false).unwrap();
    let factory = Git2ProviderFactory::new();
    let provider = factory.open(dir.path()).unwrap();
    let names: Vec<_> =
        provider.list_branches().unwrap().into_iter().map(|b| b.name).collect();
